...
import { Trainee, TrainingUser, VisualizationInfo } from '@cyberrangecz-platform/training-model';
...
export class VisualizationDefaultApi extends VisualizationApi {
  ...
  getTrainingRunInfo(trainingRunId: number): Observable<VisualizationInfo> {
    return this.http
      .get<VisualizationInfoDTO>(`${this.visualizationsEndpointUri}/${this.trainingRunUrlExtension}/${trainingRunId}`)
      .pipe(map((resp) => VisualizationInfoMapper.fromDTO(resp)));
  }
